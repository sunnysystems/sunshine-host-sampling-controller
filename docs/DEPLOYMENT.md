...
- [ ] **Labelled nodes** distinguishing the fixed fleet from the surge pool via a
      ...
      > If your node pools are organized by another axis (workload shape, team,
      > architecture) rather than fixed-vs-surge — common with Karpenter — see
      > **5.1** below.
...
### 5.1 Mapping node pools to stable/surge (Karpenter and similar)

The classification needs **one exact `key=value` label per pool** (stable and
surge). That is straightforward when a cluster has a dedicated stable pool and a
dedicated surge pool. It is **not** obvious when node pools are organized by another
axis — workload shape (High CPU / High Memory / Graviton), team, architecture —
because baseline and surge then coexist **within the same pool**, so
`karpenter.sh/nodepool` alone does not separate fixed from surge.

Two facts make this tractable:

- **Only the surge selector drives savings.** A node matching **no** selector stays
  fully monitored (untracked). You never have to enumerate the fixed pools — only
  identify the surge nodes precisely. The stable selector mostly sharpens the
  dry-run report.
- **You never label nodes by hand.** The selector points at a label the nodes
  **already** carry; Karpenter stamps its labels on every node it provisions.

Three ways to get a surge selector, from least infra change to most deterministic:

| Approach | When | Surge selector | New label? |
|----------|------|----------------|------------|
| **A. capacity-type** | Baseline on-demand, surge on spot | `karpenter.sh/capacity-type=spot` | No — Karpenter already applies it |
| **B. floorNodes** | Keep a baseline without splitting pools | an elastic pool / capacity-type | No |
| **C. dedicated surge NodePools** | Deterministic, per-pool cut | `sunshine.io/pool=surge` | Yes — on the NodePool, not per node |

- **A — capacity-type.** If baseline runs on-demand and peaks burst to spot, point
  the surge selector at `karpenter.sh/capacity-type=spot`. No new label, and it
  spans every workload pool at once.
- **B — floorNodes.** You need no physically separate "fixed" pool. Mark the elastic
  capacity as surge and set `floorNodes` to the minimum that must stay monitored.
  The controller keeps the **oldest** `floorNodes` monitored (the long-lived
  baseline in practice) and samples the newest. The cut is by **age**, not identity
  — treat `floorNodes` as a safety margin, not an exact count.
- **C — dedicated surge NodePools.** For a hard cut, split each pool into base and
  surge (e.g. `high-cpu-base` / `high-cpu-surge`) and add a shared label such as
  `sunshine.io/pool=surge` under `.spec.template.metadata.labels` of the surge
  NodePools. Karpenter stamps it on every node of that pool — still per-pool, in the
  manifest, never per node.

> The single-pair matcher cannot encode several pools in one `nodepool=` selector.
> To sample CPU, Memory and Graviton together, use capacity-type (A) or a shared
> label (C). To keep a pool (e.g. Graviton) always monitored, leave it **out** of
> the surge selector — no match means monitored.
...
**Install 1.2.0 or newer.** Earlier versions work, but each one leaves Sunshine
blinder — and the cost of that is not theoretical:

| Version | What Sunshine can see |
|---------|----------------------|
| `1.0.x` | Reconcile counts only. Honours **just the first** surge pool; the rest stay 100% monitored with no warning. |
| `1.1.0` | Honours every pool, but does not say which it applied — Sunshine cannot confirm it. |
| `1.2.0` | Reports the policy it acted on, the pools it honoured, and **the labels it sees on the nodes**. This is what lets Sunshine flag a selector that matches nothing instead of showing a serene `monitored: 0`. |
...
> **Order matters: install the controller BEFORE configuring.** From 1.2.0 the
> controller reports to Sunshine the labels it actually sees on the nodes, and
> the "how do you tell the nodes apart?" list is built from those — raw
> Kubernetes keys, badged on screen as **"from the cluster"**.
>
> Configure before any controller has reported and Sunshine falls back to
> inferring the names from **Datadog host tags**, which rewrite punctuation
> (`karpenter.sh/nodepool` arrives as `karpenter_nodepool`). The screen says so
> ("inferred from Datadog"), but a selector built from that **matches no node** —
> and fails silently, with no error on either side.
...
3. **Local lock (customer cluster):** enable the `LabelActuator` and widen RBAC:
   ...
   helm upgrade host-sampling \
     oci://ghcr.io/sunnysystems/charts/sunshine-host-sampling-controller \
     --version 1.2.1 --reuse-values --set dryRun=false
   ```
...
```sh
cosign verify ghcr.io/sunnysystems/host-sampling-controller:1.2.1 \
...
cosign verify ghcr.io/sunnysystems/charts/sunshine-host-sampling-controller:1.2.1 \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \
  --certificate-identity-regexp \
  '^https://github\.com/sunnysystems/sunshine-host-sampling-controller/\.github/workflows/release\.yml@refs/tags/v'
```
...
| **`configured: true` but `budget: 0, monitored: 0`, tick after tick** | The surge selector matches **no** node: a key that does not exist, or the right key with a value no node carries (pool renamed, or scaled to zero) | The most treacherous symptom — identical to a healthy fleet with nothing to sample, which is why it goes unnoticed for weeks. Since 1.2.0 **Sunshine names it on screen**, listing the values the cluster actually has. Confirm with `kubectl get nodes --show-labels` and `kubectl get nodepools`. |
| Monitored panel and audit trail **empty** in Sunshine, while the controller logs reconciles normally | Reports being rejected, or no token issued for this cluster | Look for `report: server rejected` in the logs. With no token, Sunshine also names it on screen since 1.2.0. A reconcile is **never** blocked by this: the effect is blindness on the console side, not risk in the cluster. |
